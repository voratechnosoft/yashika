  ...
  try {
    let {
      ...
      body: {
        ...
        arrCategoryId = [],
        arrFusionId = [],
        arrEmbroideryWorkHeightId = [],
        arrFabricPannaId = [],
        arrFabricPlainIncludedId = [],
        vDesignNumber = "",
        iPage = 1,
        iLimit = 10,
      },
    ...
    if (arrFabricId?.length > 0) {
      filter.vFabricId = { $in: arrFabricId.map((id) => new ObjectId(id)) };
    }

    if (arrCategoryId?.length > 0) {
      filter.vCategoryId = { $in: arrCategoryId.map((id) => new ObjectId(id)) };
    }
    ...
    if (arrEmbroideryWorkHeightId?.length > 0) {
      filter.vEmbroideryWorkHeightId = {
        $in: arrEmbroideryWorkHeightId.map((id) => new ObjectId(id)),
      };
    }

    if (arrFusionId?.length > 0) {
      filter.vFusionId = {
        $in: arrFusionId.map((id) => new ObjectId(id)),
      };
    }

    if (arrFabricPannaId?.length > 0) {
      filter.vFabricPannaId = {
        $in: arrFabricPannaId.map((id) => new ObjectId(id)),
      };
    }

    if (arrFabricPlainIncludedId?.length > 0) {
      filter.vFabricPlainIncludedId = {
        $in: arrFabricPlainIncludedId.map((id) => new ObjectId(id)),
      };
    }
