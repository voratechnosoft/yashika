...
const list = async (entry) => {
  try {
    ...
    let dataList = await dbService.aggregateData("FusionModel", aggregateQuery);

    let totalCount = await dbService.recordsCount("FusionModel", filter);
